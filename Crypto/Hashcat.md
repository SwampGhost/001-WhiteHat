...
`.\hashcat --identify {hash} OR {pathToHashFile}`

Code to decrypt using GPU optimized
`.\hashcat -D 2 -m {algo id} {hash} OR {pathToHashFile} {pathToDicktionary} -O -w 2`

`e.x. .\hashcat.exe -D 2 -m 1800 hash/theGrinch.txt rockyou.txt -O -w 4`
##optimized, nightmare headless on SHA512 with S6 salt - 14million compars in < 1 min

Run benchmarks
`.\hashcat -b`

Decrypt SHA512 S6 PWD with rules - rockyou.rule  will add rockyou.txt
`.\hashcat.exe -D 2 -m 1800 hash/theGrinch.txt rockyou.txt -O -w 4 -a 9 -r rules\rockyou-30000.rule`

---
```
hashcat (v6.2.2) starting in autodetect mode...
